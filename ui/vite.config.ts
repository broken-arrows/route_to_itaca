...
export default defineConfig({
  ...
  server: {
    fs: {
      ...
      // `out/html/cat_engine.js` — deliberately shared VERBATIM with the old UI
      // so there is exactly one copy of the model (see that file's header).
      // It lives outside this Vite root, and `ui/package-lock.json` makes Vite's
      // workspace-root search stop AT `ui/`, so the default fs.allow does not
      // cover it: a cold request for `/@fs/…/out/html/cat_engine.js` 403s.
      // (It happens to succeed once the module graph is warm from `/src/main.ts`,
      // which is an implementation detail of Vite's serving check — do not rely
      // on it.) Allow the repo root explicitly. Dev-server only; the production
      // build resolves the import through Rollup and needs nothing here.
      allow: ['..'],
    },
  },
  optimizeDeps: {
  ...
  build: {
    ...
    // detected in the production build.
    commonjsOptions: {
